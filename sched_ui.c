...
typedef struct {
    int n, activeField, bufLen, arrival[MAX_PROCESSES], burst[MAX_PROCESSES];
    char buf[16];
}InputState;

static void InputState_init(InputState *s){
    *s = (InputState){
        .n = 3, .activeField = -1, .arrival = {0,2,4}, .burst = {5,4,6},
    };
}
...
static int HandleInput(InputState *s, int SW, int SH) {
    ...
    if (IsKeyPressed(KEY_UP) && s->n < MAX_PROCESSES) {
        if (++s->n, s->activeField != -1 && s->activeField/2 >= s->n) s->activeField = -1;
    }
...
if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    ...
    for (int i = 0; i < s->n; i++) {
        Rectangle cells[2] = { {320, 180 + i*50, 140, 34},   /*arrival*/ {500, 180 + i*50, 140, 34},   /* burst*/ };
            for (int c = 0; c < 2; c++) {
...
static void DrawCell(const InputState *s, int fieldIdx, int cx, int y) {
    ...
    //blinking cursor
    if (active && ((int)(GetTime()*2) % 2 == 0)) FDraw("|", cx + 8 + FMeasure(val, 16), y + 8, 16, COL_ACCENT);
}
...
void RunScheduler(const char *windowTitle, const char *algoLabel, ComputeFn compute) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (state == 1 && (IsKeyPressed(KEY_BACKSPACE) || IsKeyPressed(KEY_ESCAPE))) state = 0;
